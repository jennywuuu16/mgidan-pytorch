...
def topk_candidate_mask(score, top_k, include_self=False):
...
def build_candidate_mask(prior_similarity, target_similarity, top_k, mode='full'):
    if prior_similarity.shape != target_similarity.shape:
        raise ValueError('prior_similarity and target_similarity must have the same shape')
    n = prior_similarity.shape[0]
    if mode == 'full':
        mask = torch.ones((n, n), dtype=torch.bool, device=prior_similarity.device)
        mask.fill_diagonal_(False)
        return mask
    prior = topk_candidate_mask(prior_similarity, top_k=top_k)
    target = topk_candidate_mask(target_similarity, top_k=top_k)
    if mode == 'prior_topk':
        return prior
    if mode == 'target_topk':
        return target
    if mode == 'hybrid':
        return prior | target
    raise ValueError('mode must be one of: full, prior_topk, target_topk, hybrid')
