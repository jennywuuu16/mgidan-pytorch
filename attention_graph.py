...
class PaperNodeEmbedding(nn.Module):
...
class PaperAttentionScorer(nn.Module):
...
class EndToEndAttentionGraphConv(nn.Module):
...
def smoke_test(device: str='cpu') -> None:
    set_random_seed()
    n, seq_len, seq_dim, static_dim = (8, 6, 3, 4)
    model = EndToEndAttentionGraphConv(seq_dim=seq_dim, static_dim=static_dim, sales_hidden_dim=16, char_hidden_dim=12, embed_dim=10, attn_dim=8, graph_dim=10, dropout=0.0).to(device)
    seq_x = torch.randn(n, seq_len, seq_dim, device=device)
    static_x = torch.randn(n, static_dim, device=device)
    ...
    output = model(seq_x, static_x, candidate_mask, return_undirected_edge_weight=True)
    assert output.node_embedding.shape == (n, 10)
    assert output.alpha.shape == (n, n)
    assert output.neighbor_embedding.shape == (n, 10)
    assert output.graph_embedding.shape == (n, 10)
    assert output.undirected_edge_weight is not None
    assert output.undirected_edge_weight.shape == (n, n)
    effective_mask = candidate_mask.clone()
    empty_rows = ~effective_mask.any(dim=1)
    if empty_rows.any():
        idx = torch.arange(n, device=device)
        effective_mask[empty_rows, idx[empty_rows]] = True
    row_sums = output.alpha.sum(dim=1)
    assert torch.allclose(row_sums, torch.ones_like(row_sums), atol=1e-06)
    assert torch.all(output.alpha.masked_select(~effective_mask) == 0)
    forecast_head = nn.Linear(10, 1).to(device)
    y = torch.randn(n, 1, device=device)
    pred = forecast_head(output.graph_embedding)
    loss = F.mse_loss(pred, y)
    loss.backward()
    grad_checks = {'sales_encoder': model.node_embedding.sales_encoder.weight_ih_l0.grad, 'char_encoder': model.node_embedding.char_encoder[0].weight.grad, 'query_proj': model.attention_scorer.query_proj.weight.grad, 'key_proj': model.attention_scorer.key_proj.weight.grad, 'attn_vector': model.attention_scorer.attn_vector.grad, 'graph_update': model.aggregator.update[0].weight.grad}
    ...
    print(f'alpha_row_sum_min={float(row_sums.min().detach().cpu()):.6f}')
    print(f'alpha_row_sum_max={float(row_sums.max().detach().cpu()):.6f}')

def main() -> None:
    parser = argparse.ArgumentParser(description='Smoke test the end-to-end attention graph component.')
